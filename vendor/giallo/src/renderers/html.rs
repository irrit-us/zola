...
pub struct HtmlRenderer {
    /// Emit escaped text directly for tokens with the complete default style.
    #[serde(default)]
    ...
    pub fn render(&self, highlighted: &HighlightedCode, options: &RenderOptions) -> String {
        ...
        while let Some((idx, line_tokens)) = tokens.next() {
            ...
            for tok in line_tokens {
                let is_plain = match (&tok.style, &highlighted.theme) {
                    (ThemeVariant::Single(style), ThemeVariant::Single(theme)) => {
                    ...
                    ) => *light == lt.default_style && *dark == dt.default_style,
                    _ => false,
                };
                if self.omit_plain_spans && is_plain {
                    line_content.push(HtmlEscaped(&tok.text).to_string());
                } else {
